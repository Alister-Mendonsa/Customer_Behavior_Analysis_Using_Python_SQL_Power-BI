...
select item_purchased, ROUND(avg(review_rating::numeric),2) from 
customer
...
as avg_spend , sum(purchase_amount) as total_revenue
from  customer
...
select item_purchased,ROUND(100 * sum(case when  discount_applied = 'Yes' then 1 else  0 END)/count(*)
,2) as discount_rate from customer
group by item_purchased
